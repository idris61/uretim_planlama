frappe.ui.form.on("Sales Order", {
	refresh: function (frm) {
		if (frm.fields_dict["custom_raw_materials_html"]) {
			...
			const btnRawMaterials = document.getElementById(
				"btn-raw-materials-show",
			);
			const btnLongTermReserve = document.getElementById(
				"btn-long-term-reserve-check",
			);
			const btnClearLongTermReserve = document.getElementById(
				"btn-clear-long-term-reserve",
			);

			if (btnRawMaterials) {
				btnRawMaterials.onclick = function () {
					const tableContentDiv = document.getElementById(
						"raw-materials-table-content",
					);
					if (!frm.doc.name || frm.doc.__islocal) {
						frappe.msgprint(
							"Lütfen önce Satış Siparişini kaydedin.",
						);
						return;
					}
					tableContentDiv.innerHTML =
						'<div style="padding:24px;text-align:center;color:#888;font-size:16px;">Yükleniyor...</div>';
			...
			if (frm.doc.is_long_term_child || frm.doc.parent_sales_order) {
				if (btnLongTermReserve)
					btnLongTermReserve.style.display = "none";
				if (btnClearLongTermReserve)
					btnClearLongTermReserve.style.display = "none";
			} else {
				if (btnLongTermReserve) btnLongTermReserve.style.display = "";
				if (btnClearLongTermReserve)
					btnClearLongTermReserve.style.display = "";
			}
			if (btnLongTermReserve) {
				btnLongTermReserve.onclick = function () {
					...
					if (frm.doc.docstatus == 1) {
						frappe.msgprint(
							"Onaylanmış (submit edilmiş) satış siparişlerinde uzun vadeli rezerv kullanılamaz.",
						);
						return;
					}
					// Child siparişlerde buton zaten gizli, yine de koruma
					if (
						frm.doc.is_long_term_child ||
						frm.doc.parent_sales_order
					) {
						frappe.msgprint(
							"Bu sipariş ana siparişe bağlıdır. Uzun vadeli rezerv kullanımı ana siparişten yönetilmelidir.",
						);
						return;
					}
							...
							frappe.call({
								...
								callback: function (r) {
									if (r.message && r.message.success) {
										frappe.show_alert({
											message:
												"Kalan uzun vadeli rezervler silindi!",
											indicator: "green",
										});
										loadRawMaterialsTable(frm);
									}
	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message && r.message.length > 0) {
				...
				frappe.show_alert({
					message:
						"Uzun vadeli rezervden kullanılabilecek hammadde bulunamadı.",
					indicator: "orange",
				});
			}
		},
		...
		callback: function (r) {
			if (r.message) {
				...
				r.message.forEach((row) => {
					...
					let uzunVadeliRezerv =
						parseFloat(row.long_term_reserve_qty) || 0;
					let kullanilanRezerv =
						parseFloat(row.used_from_long_term_reserve) || 0;

					if (
						acikMiktar > 0 &&
						uzunVadeliRezerv - kullanilanRezerv > 0
					) {
						...
						shortageItems.push({
							...
							kullanilabilir_uzun_vadeli:
								uzunVadeliRezerv - kullanilanRezerv,
						});
					}
				});
				...
				if (frm.doc.is_long_term_child || frm.doc.parent_sales_order) {
					...
					hasShortageWithLongTermReserve &&
					frm.doc.docstatus != 1
				) {
					html += `
				...
				let rows = r.message.map((row) => {
					let rowClass = "";
					let acikMiktar = parseFloat(row.acik_miktar) || 0;
					let kullanilabilirStok =
						parseFloat(row.kullanilabilir_stok) || 0;
				...
				rows.sort(
					(a, b) =>
						(renkOncelik[a.rowClass] || 99) -
						(renkOncelik[b.rowClass] || 99),
				);
				...
				rows.forEach(({ row, rowClass }) => {
					let acikMiktar,
						...
						kullanilanRezerv;
					let acikMiktarCell,
						kullanilabilirStokCell,
						toplamRezervCell,
						uzunVadeliRezervCell,
						kullanilanRezervCell,
						malzemeTalepCell,
						siparisEdilenCell,
						beklenenTeslimCell,
						qtyFormatted,
						reservedQtyFormatted,
						itemLink,
						itemNameLink,
						stokTooltip,
						stokCell,
					...
					uzunVadeliRezerv =
						parseFloat(row.total_long_term_reserve_qty) || 0;
					kullanilanRezerv =
						parseFloat(row.used_from_long_term_reserve) || 0;
					// Child siparişlerde özel gösterim (backend'den gelen alanlara göre)
					let isLongTermChild =
						row.is_long_term_child || row.parent_sales_order;
					if (isLongTermChild) {
						kullanilabilirStokCell =
							"<span title='Ana rezervden karşılanacak' style='color:#888;cursor:help;'>-</span>";
						acikMiktarCell =
							"<span title='Ana rezervden karşılanacak' style='color:#888;cursor:help;'>-</span>";
					} else {
						acikMiktarCell =
							Math.abs(acikMiktar) > 0.001
								? `<span class='acik-miktar'>${acikMiktar.toFixed(2)}</span>`
								: `<span class='stok-pozitif-box'>0</span>`;
						kullanilabilirStokCell =
							kullanilabilirStok > 0.001
								? `<span class='stok-pozitif-box'>${kullanilabilirStok.toFixed(2)}</span>`
								: `<span class='stok-negatif'>${kullanilabilirStok.toFixed(2)}</span>`;
					}
					...
					itemNameLink = row.item_name
						? `<a href='/app/item/${encodeURIComponent(row.raw_material)}' target='_blank'>${row.item_name}</a>`
						: "";
					stokTooltip =
						Object.keys(stok_by_warehouse).length > 0
					...
					ihtiyacCell =
						qtyFormatted > 0
							? `<span class='ihtiyac-turuncu'>${qtyFormatted}</span>`
							: qtyFormatted;
				...
				setTimeout(function () {
					...
					document
						.querySelectorAll(".long-term-detail-link")
						.forEach((el) => {
							el.onclick = function (e) {
								e.preventDefault();
								let itemCode = el.getAttribute("data-item");
								let row = r.message.find(
									(rw) => rw.raw_material === itemCode,
								);
								if (
									row &&
									row.long_term_details &&
									row.long_term_details.length > 0
								) {
									showDetailsModal(
										__("Long Term Reserve Details"),
										row.long_term_details,
										[
											__("Sales Order"),
											__("Customer"),
					...
					document
						.querySelectorAll(".used-long-term-detail-link")
						.forEach((el) => {
							el.onclick = function (e) {
								e.preventDefault();
								let itemCode = el.getAttribute("data-item");
								...
								if (
									row &&
									row.used_long_term_details &&
									row.used_long_term_details.length > 0
								) {
									showDocumentDetailsModal(
										__("Used Long Term Reserve Details"),
										row.used_long_term_details,
										[
											__("Sales Order"),
											__("Customer"),
					...
					document
						.querySelectorAll(".malzeme-talep-detail-link")
						.forEach((el) => {
							el.onclick = function (e) {
								e.preventDefault();
								let itemCode = el.getAttribute("data-item");
					...
					document
						.querySelectorAll(".siparis-edilen-detail-link")
						.forEach((el) => {
							el.onclick = function (e) {
								e.preventDefault();
								let itemCode = el.getAttribute("data-item");
					...
					document
						.querySelectorAll(".total-reserved-detail-link")
						.forEach((el) => {
							el.onclick = function (e) {
								e.preventDefault();
								let itemCode = el.getAttribute("data-item");
								...
								if (
									row &&
									row.total_reserved_details &&
									row.total_reserved_details.length > 0
								) {
									showDetailsModal(
										__("Toplam Rezerv Detayı"),
										row.total_reserved_details,
										[
											__("Sales Order"),
											__("Customer"),
...
function enrichDetailsWithEndCustomer(details) {
	...
	return details.map((detail) => {
		if (
			(!detail.custom_end_customer ||
				detail.custom_end_customer === "") &&
			detail.sales_order &&
			soToEndCustomer[detail.sales_order]
		) {
			return {
				...detail,
				custom_end_customer: soToEndCustomer[detail.sales_order],
			};
		}
		return detail;
	});
...
function showDetailsModal(title, details, columns, type) {
	if (
		columns.includes(__("End Customer")) ||
		columns.includes(__("Nihayi Müşteri"))
	) {
...
function renderDetailsModal(title, details, columns, doctype) {
	...
	if (!details || details.length === 0) {
		...
		details.forEach((detail) => {
			...
			// Child sipariş mi?
			let isChild = false;
			if (detail.is_long_term_child || detail.parent_sales_order)
				isChild = true;
			...
			let rowStyle = "";
			if (isLongTerm)
				rowStyle = "background:#e3f2fd;color:#1976d2;font-weight:bold;";
			if (isChild)
				rowStyle = "background:#fff3e0;color:#ff9800;font-weight:bold;";
			if (isChildUsage)
				rowStyle = "background:#fff3e0;color:#ff9800;font-weight:bold;";
			...
			columns.forEach((col) => {
				...
				if (
					...
					if (!doctype_link) {
						if (col.includes("Satış") || col.includes("Sales"))
							doctype_link = "sales-order";
						else if (col.includes("Material"))
							doctype_link = "material-request";
						else if (col.includes("Purchase"))
							doctype_link = "purchase-order";
						else doctype_link = "";
					}
					...
					col === "Miktar" ||
					col === "Quantity" ||
					col === __("Kullanılan Miktar") ||
					col === __("Used Qty") ||
					col === "Kullanılan Miktar" ||
					col === "Used Qty"
				) {
...
function showLongTermReserveUsageSummaryModal(parent_sales_order) {
	frappe.call({
		...
		callback: function (r) {
			...
			let toplam_kullanilan =
				parseFloat(r.message.toplam_kullanilan) || 0;
			...
			if (document.getElementById("longTermReserveUsageSummaryModal"))
				document
					.getElementById("longTermReserveUsageSummaryModal")
					.remove();
			document.body.insertAdjacentHTML("beforeend", modalHtml);
