frappe.pages['accessory_delivery_package_create'].on_page_load = function(wrapper) {
   ...
   frappe.call({
        method: 'uretim_planlama.api.get_approved_opti_nos',
        callback: function(r) {
            let $select = $('#adp_opti_no');
            $select.empty().append('<option value="">Seçiniz</option>');
            (r.message || []).forEach(function(row) {
                if (row.custom_opti_no) {
        // OpTi No dropdown'ı doldur
                 $select.append(`<option value="${row.name}">${row.custom_opti_no}</option>`);
                }
            });
        }
    });
...
$(document).on('change', '#adp_opti_no', function() {
    ...
    if (!opti_no) return;
    frappe.call({
        method: 'uretim_planlama.api.get_sales_orders_by_opti',
...
$(document).on('click', '#adp_search_btn', function() {
    ...
    frappe.call({
        method: 'uretim_planlama.api.get_materials_by_opti',
        args: { opti_no: opti_no },
        callback: function(r) {
            let materials = r.message && r.message.materials ? r.message.materials : [];
            let sales_orders = r.message && r.message.sales_orders ? r.message.sales_orders : [];
            if (sales_orders.length && sales_orders.indexOf(sales_order) === -1) {
                $alert.html('<div class="alert alert-warning">Seçili OpTi No için bu satış siparişi bulunamadı.</div>').show();
                return;
            }
...
$(document).on('click', '#adp_create_btn', function() {
    ...
    frappe.call({
        method: 'uretim_planlama.api.create_delivery_package',
        args: {
            data: {
                opti_no: opti_no,
