...
frappe.pages['hammadde-eksikleri-raporu'].on_page_load = function(wrapper) {
...
class HammaddeEksikleriRaporu {
	...
	make() {
		...
		this.container = $('<div class="hammadde-eksikleri-raporu"></div>').appendTo(this.page.main);
	...
	loadData() {
		...
		this.loadingRequest = frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_all_shortages_report',
			args: {
				filters: me.filters
			},
			callback: function(r) {
	...
	showReserveDetails(itemCode) {
		...
		frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_reserve_details',
	...
	showMaterialRequestDetails(itemCode) {
		...
		frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_material_request_details',
			args: { item_code: itemCode },
			callback: function(r) {
				if (r.message && r.message.success) {
	...
	showPurchaseOrderDetails(itemCode) {
		...
		frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_purchase_order_details',
			args: { item_code: itemCode },
			callback: function(r) {
				if (r.message && r.message.success) {
	...
	showWarehouseStockModal(itemCode, itemName) {
		...
		frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_warehouse_stock_details',
			args: { item_code: itemCode },
			callback: function(r) {
	...
	loadProfileLengthData() {
		...
		frappe.call({
			method: 'uretim_planlama.uretim_planlama.page.hammadde_eksikleri_raporu.hammadde_eksikleri_raporu.get_profile_length_shortages',
			args: {
