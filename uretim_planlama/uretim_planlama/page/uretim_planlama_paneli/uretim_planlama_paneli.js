...
(function () {
	...
	class UretimPlanlamaPaneli {
		...
		updateOrderDetailsModal(data, modal) {
			...
			const serial = (firstItem.seri || '').toString().trim();
			const color = (firstItem.renk || '').toString().trim();
		...
		`;
			...
			// ✅ VERİ YOKSA SATIRI GİZLE (SİLMEDEN)
			if (!serial) $(modal).find('#row-serial').hide();
			if (!color) $(modal).find('#row-color').hide();
		}
