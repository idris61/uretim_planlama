        ...
        unplanned_query = """
            SELECT
                ...
                CASE
                    ...
                    ELSE 0
                END as cam_qty,
                -- Camlar: total_mtul = TM2 (zaten adet dahil), adet ile tekrar çarpma. PVC/diğer: mtul = adet * birim
                CASE
                    WHEN (i.item_group = 'Camlar' OR i.custom_stok_türü = 'Camlar')
                    THEN COALESCE(i.custom_amount_per_piece, 0)
    ...
    try:
        ...
        rows = frappe.db.sql(
            ...
            SELECT
                ...
                -- total_mtul (liste ile aynı): Camlar = TM2 (adet dahil), PVC/diğer = adet * birim
                CASE
                    WHEN (i.item_group = 'Camlar' OR i.custom_stok_türü = 'Camlar')
                    THEN COALESCE(i.custom_amount_per_piece, 0)
                    WHEN i.custom_amount_per_piece IS NOT NULL AND i.custom_amount_per_piece > 0
