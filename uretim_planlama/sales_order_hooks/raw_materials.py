...
def get_usage_map(item_codes):
    ...
    for row in usage_rows:
        ...
        if row.parent_sales_order:
            usage_map[(row.parent_sales_order, row.item_code)] = usage_map.get((row.parent_sales_order, row.item_code), 0) + normalize_qty(row.used_qty)
        # Bağımsız usage kayıtları için (None, item_code) anahtarları da ekle
        if not row.sales_order:
            usage_map[(None, row.item_code)] = usage_map.get((None, row.item_code), 0) + normalize_qty(row.used_qty)
    return usage_map
...
def get_stock_maps(item_codes):
    ...
    for row in bin_rows:
        ...
        stock_by_warehouse_map.setdefault(row.item_code, {})[row.warehouse] = normalize_qty(row.actual_qty)
...
def get_sales_order_raw_materials(sales_order):
    ...
    total_long_term_map = {row.item_code: normalize_qty(row.total_long_term) for row in total_long_term_rows}
    ...
    for item_code, item_pairs in item_bom_map.items():
        ...
        if is_long_term_child and parent_sales_order:
            ...
            used_from_long_term_reserve = usage_map.get(long_term_key, 0) + usage_map.get((None, item_code), 0)
        ...
        for item, rm in item_pairs:
            raw_materials[item_code]["qty"] += rm.qty * item.qty
    ...
    for data in raw_materials.values():
        ...
        for d in long_term_details:
            if not d.get('custom_end_customer') and d.get('sales_order'):
                d['custom_end_customer'] = frappe.db.get_value('Sales Order', d['sales_order'], 'custom_end_customer') or ''
...
def get_long_term_reserve_details(item_code):
    ...
    """
    thirty_days_later = (datetime.strptime(frappe.utils.nowdate(), "%Y-%m-%d") + timedelta(days=30)).strftime("%Y-%m-%d")
    rows = frappe.db.sql('''
    ...
    ''', (item_code, thirty_days_later), as_dict=True)
    # Eksik custom_end_customer varsa, satış siparişinden çek
    for row in rows:
        if not row.get('custom_end_customer') and row.get('sales_order'):
            row['custom_end_customer'] = frappe.db.get_value('Sales Order', row['sales_order'], 'custom_end_customer') or ''
    return rows
...
def upsert_reserved_raw_material(sales_order, item_code, qty, item_name, customer, end_customer):
    """
...
def create_reserved_raw_materials_on_submit(doc, method):
    ...
    try:
        ...
        if getattr(doc, "is_long_term_child", 0) or getattr(doc, "parent_sales_order", None):
            return
...
def delete_reserved_raw_materials_on_cancel(doc, method):
    try:
        ...
        if getattr(doc, "is_long_term_child", 0) or getattr(doc, "parent_sales_order", None):
            print(f"[DEBUG] Child sipariş iptalinde rezerve hammaddeler silinmeyecek: {getattr(doc, 'name', None)}")
            return
...
def delete_long_term_reserve_usage_on_cancel(doc, method):
    ...
    try:
        ...
        for row in usage_rows:
            ...
            if is_long_term_child and parent_sales_order:
                parent_reserve = frappe.get_all(
                    "Rezerved Raw Materials",
                    filters={"sales_order": parent_sales_order, "item_code": row["item_code"]},
                    fields=["name", "quantity"]
                )
                if parent_reserve:
                    reserve_doc = frappe.get_doc("Rezerved Raw Materials", parent_reserve[0]["name"])
                    reserve_doc.quantity = normalize_qty(reserve_doc.quantity + float(row["used_qty"]))
        ...
        frappe.db.commit()
        frappe.msgprint(_("{0} için uzun vadeli rezerv kullanımları silindi ve parent rezervler geri yüklendi.").format(sales_order), alert=True, indicator="orange")
    except Exception:
...
def upsert_long_term_reserve_usage(sales_order, item_code, qty, is_long_term_child, parent_sales_order):
    """
    ...
    try:
        ...
        for usage in usage_list:
            ...
            if is_long_term_child and parent_sales_order:
                ...
                ''', (item_code,))[0][0])
            kalan_kullanilabilir = max(parent_rezerv - total_usage, 0)
            frappe.logger().info(f"[DEBUG] item_code={item_code}, qty={qty}, parent_rezerv={parent_rezerv}, total_usage={total_usage}, kalan_kullanilabilir={kalan_kullanilabilir}")
            if qty - kalan_kullanilabilir > EPSILON:
                return {"success": False, "message": f"{item_code} için toplam uzun vadeli rezerv miktarı aşılamaz! (Kalan: {kalan_kullanilabilir})"}
            if is_long_term_child and parent_sales_order:
...
def get_long_term_reserve_usage_summary(parent_sales_order):
    ...
    """, (parent_sales_order,), as_dict=True)
    return ana_rezerv
...
def delete_long_term_reserve_usage_on_delivery_or_invoice(doc, method):
    ...
    try:
        ...
        frappe.log_error("Delivery/Invoice/StockEntry/WorkOrder hook (delete_long_term_reserve_usage_on_delivery_or_invoice) HATA", frappe.get_traceback())
...
def restore_reserved_raw_materials_on_cancel(doc, method):
    ...
    try:
        ...
        for row in reserved:
            frappe.delete_doc("Rezerved Raw Materials", row["name"], ignore_permissions=True)
        frappe.db.commit()
        frappe.msgprint(_("{0} için rezerve iptal edildi.").format(sales_order), alert=True, indicator="orange")
    except Exception:
        frappe.log_error("Sales Order cancel hook (restore_reserved_raw_materials_on_cancel) HATA", frappe.get_traceback())

def handle_child_sales_order_reserves(doc, method):
    print(f"[DEBUG] handle_child_sales_order_reserves ÇAĞRILDI: {getattr(doc, 'name', None)} is_long_term_child={getattr(doc, 'is_long_term_child', None)} parent_sales_order={getattr(doc, 'parent_sales_order', None)}")
    if not getattr(doc, "is_long_term_child", 0) or not getattr(doc, "parent_sales_order", None):
        print(f"[DEBUG] Child kontrolü geçilemedi: is_long_term_child={getattr(doc, 'is_long_term_child', None)}, parent_sales_order={getattr(doc, 'parent_sales_order', None)}")
        return
    ...
    for item in doc.items:
        ...
        for rm in bom_doc.items:
            ...
            toplam_ihtiyac[hammadde_code] = normalize_qty(toplam_ihtiyac.get(hammadde_code, 0) + ihtiyac)
    # 2. Her hammadde için parent rezervde yeterli miktar var mı kontrol et
    for hammadde_code, ihtiyac in toplam_ihtiyac.items():
        parent_reserve = frappe.get_all(
            "Rezerved Raw Materials",
            filters={
                "sales_order": parent_so,
        ...
        if parent_reserve:
            ...
            print(f"[DEBUG] Hammadde toplu kontrol: hammadde_code={hammadde_code}, parent_rezerv={mevcut_rezerv}, toplam_ihtiyac={ihtiyac}")
            if mevcut_rezerv >= ihtiyac:
                ...
                if normalize_qty(reserve_doc.quantity) <= 0:
                    ...
                    reserve_doc.save(ignore_permissions=True)
                print(f"[DEBUG] Parent rezerv güncellendi: {parent_reserve[0]['name']}, kalan={reserve_doc.quantity}")
                print(f"[DEBUG] USAGE KAYDI OLUŞTURULUYOR: child_so={doc.name}, hammadde_code={hammadde_code}, qty={ihtiyac}, parent_so={parent_so}")
                upsert_long_term_reserve_usage(
                    sales_order=doc.name,
                    item_code=hammadde_code,
                    qty=ihtiyac,
                ...
                print(f"[DEBUG] Yetersiz rezerv: parent_so={parent_so}, hammadde_code={hammadde_code}, mevcut_rezerv={mevcut_rezerv}, toplam_ihtiyac={ihtiyac}")
                frappe.throw(_(f"Ana siparişte {hammadde_code} için yeterli uzun vadeli rezerv yok! (Gereken: {ihtiyac}, Mevcut: {mevcut_rezerv})"))
        else:
            print(f"[DEBUG] Yetersiz rezerv: parent_so={parent_so}, hammadde_code={hammadde_code}, mevcut_rezerv=YOK")
            frappe.throw(_(f"Ana siparişte {hammadde_code} için yeterli uzun vadeli rezerv yok!"))
    frappe.db.commit()
